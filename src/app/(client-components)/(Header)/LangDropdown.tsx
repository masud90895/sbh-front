			...
			<div className="grid gap-7 lg:grid-cols-2">
				{headerCurrency.map((item, index) => (
					<a
						...
						href={item.href}
						onClick={() => close()}
						className={`-m-3 flex items-center rounded-lg p-2 hover:bg-gray-100 focus:outline-none focus-visible:ring focus-visible:ring-orange-500 focus-visible:ring-opacity-50 dark:hover:bg-gray-700 ${
							item.active ? 'bg-gray-100 dark:bg-gray-700' : 'opacity-80'
						}`}
					>
				...
				{({ open, close }) => (
					<>
						...
						>
							...
							<ChevronDownIcon
								className={`${open ? '-rotate-180' : 'text-opacity-70'} ms-1 h-4 w-4 group-hover:text-opacity-80`}
								aria-hidden="true"
							/>
						</PopoverButton>
						<Transition
						...
						>
							<PopoverPanel className={`absolute z-20 ${panelClassName}`}>
								<div className="rounded-2xl bg-white p-3 shadow-lg ring-1 ring-black ring-opacity-5 dark:bg-neutral-800 sm:p-6">
									<TabGroup>
										<TabList className="flex gap-x-1 rounded-full bg-gray-100 p-1 dark:bg-slate-700">
