...
const NavigationItem: FC<NavigationItemWithRouterProps> = ({ menuItem }) => {
	...
	const renderMegaMenu = (menu: NavItemType) => {
		...
		const classPanel = isFull ? 'start-0' : '-translate-x-1/2 left-1/2'
				...
				{() => (
					<>
						...
						>
							<PopoverPanel
								static
								className={`sub-menu absolute top-full z-10 w-screen max-w-sm px-4 sm:px-0 lg:max-w-max ${classPanel}`}
							>
				...
				{() => (
					<>
						...
						>
							<PopoverPanel
								static
								className="sub-menu absolute start-0 top-full z-10 w-56"
							>
				...
				{() => (
					<>
						...
						>
							<PopoverPanel
								static
								className="sub-menu absolute start-full top-0 z-10 w-56 ps-2"
							>
								<ul className="relative grid space-y-1 rounded-lg bg-white py-4 text-sm shadow-lg ring-1 ring-black ring-opacity-5 dark:bg-neutral-900 dark:ring-white dark:ring-opacity-10">
									{item.children?.map((i) => {
										if (i.type) {
