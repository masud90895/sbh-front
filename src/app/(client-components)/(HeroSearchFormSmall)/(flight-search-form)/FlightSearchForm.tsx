				...
				<Popover className="relative">
					{({ open }) => (
						<>
							...
							>
								<span>{`${totalGuests || ''} ${T['HeroSearchForm']['Guests']}`}</span>
								<ChevronDownIcon
									className={`${
										open ? '' : 'text-opacity-70'
									} ms-2 h-4 w-4 group-hover:text-opacity-80`}
									aria-hidden="true"
								/>
							</PopoverButton>
							<Transition
					...
					{({ open, close }) => (
						<>
							...
							<Transition
								...
								leaveTo="opacity-0 translate-y-1"
							>
								<PopoverPanel className="absolute left-1/2 z-30 mt-3 w-screen max-w-[200px] -translate-x-1/2 px-4 sm:max-w-[220px] sm:px-0">
										...
										<div className="relative grid bg-white p-3 dark:bg-neutral-800">
											{flightClass.map((item) => (
												<a
													...
													className="flex items-center rounded-lg p-2 hover:bg-neutral-100 focus:outline-none focus-visible:ring focus-visible:ring-orange-500 focus-visible:ring-opacity-50 dark:hover:bg-gray-700"
												>
